...
(function() {
    ...
    //     if(tp){
    //         if(tp == 'Y'){　return year;　}
    //         if(tp == 'M'){ return month; }
    //         if(tp == 'D'){ return date; }
    //         if(tp == 'h'){ return hour; }
    //         if(tp == 'm'){ return minute; }
    //         if(tp == 's'){ return second; }
    //         if(tp == 'all'){
    //             return {
    //                 yeat : year,
    //                 month : month,
    ...
    //     }
    // };
    // D.time = new Date(D.nowTime * 1000).getTime() / 1000;
    D.clock = D.clock || {};
    D.clock.date = '';
    D.clock.dom = '',
    D.clock.timeout = false;
    ...
    /**
     * [serverClock description]
     * @param  {[type]} s_year  [description]
     * @param  {[type]} s_month [description]
     * @param  {[type]} s_day   [description]
     * @param  {[type]} s_hour  [description]
     * @param  {[type]} s_min   [description]
     * @param  {[type]} s_sec   [description]
     * @return {[type]}         [description]
     */
    // D.clock.serverClock = function(s_year, s_month, s_day, s_hour, s_min, s_sec) {
    D.clock.serverClock = function(s_date) {
        var _date = s_date.replace(/-/g, '/');
     ...
     *     date ； '2014-1-1 14:00',
     *     dom : $(),
     *     tpl : '$YY$年$MM$月$DD$日 $mm$:$ss$'
     * }
     */
    D.clock.start = function(opt) {

        var day = (opt && typeof(opt) == 'string' ? opt : opt.date);
        var time = {};

        var srvClock = new D.clock.serverClock(day);

        // var srvClock = new D.clock.serverClock(time.year, time.month, time.day, time.hour, time.minute, time.second);

        this.timeout = setInterval(function() {

            var _format = opt.tpl ? opt.tpl : 'yyyy-MM-dd HH:mm:ss';
            // var html = srvClock.get_ServerTime('yyyy年MM月dd日 HH:mm:ss');
            var html = srvClock.get_ServerTime(opt.tpl);
            if (opt.endTime) {
                var endClock = new D.clock.serverClock(opt.endTime);
                var endHtml = endClock.get_ServerTime(opt.tpl);
                var compareTime = endClock.get_ServerTime().getTime() - srvClock.get_ServerTime().getTime();
                if (compareTime < 0) {
                    // xue.log('end.....');
                    if (opt.endCallback) {
                        opt.endCallback(endHtml);
                    }
                    D.clock.stop();
                    return endHtml;
                } else {
                    if (opt.callback) {
                        opt.callback(compareTime);
                    }
                }
            } else {
                if (opt.callback) {
...
(function(){
    if(xue.isMobile){
        // xue.log($.cookie('xesapp_isread'));
        if($.cookie('xesapp_isread') == 'ok'){
            $(function(){

            // if($('#xesAppJoinTips').length === 0){
                var tpl = '<style>#xesAppJoinTips img{ position:fixed; bottom:0; z-index: 10000; display:block; height:auto; width:100%; } </style><div id="xesAppJoinTips"><a href="//jzh.xueersi.com/down.html"><img src="http://img04.xesimg.com/join_xesapp_tips.png" alt="" /></a></div>';
                $('body').append(tpl);
            // }
            });
            
        }else{
            window.location.href = 'http://www.xueersi.com/landing.html';
        }
    }
})();
...
$(function() {
    ...
    $('#down_link').on('click', function(){
        if (/android|Linux/i.test(navigator.userAgent)){
            if(xue.isWeChat){
                $('body').append('<p class="speech">我很努力，但微信不让下载。<br />请点击<span class="text-danger">右上角</span>，选择“<span class="text-danger">在浏览器中打开</span>”就能下载了。</p>');
            }else{
    ...
    $('#header .msg_tips a').on('click', function() {
        ...
        var href = {
            url: loc.pathname,
            tp: loc.href.split('#_')[1]
        };
        // 如果不是消息页面，直接跳转
        if (link.url !== href.url) {
            return true;
        }
...
// })();
// $(function(){
// xue.log($("script[src='/js/xue.examination.min.js']").length)
// $("script[src='/js/xue.examination.min.js']").remove();
// $("script[src='/js/xue.page.examination.js']").remove();
// xue.loader('http://js04.xesimg.com/xue.exam.min.js', function(){
//     xue.loader('http://js04.xesimg.com/xue.page.exam.js', function(){
//     });
// });
// });
...
(function($) {
    $.fn.mailAutoComplete = function(options) {
        ...
        $(this).each(function() {
            ...
            //样式的初始化
            that.wrap('<span style="display:inline-block;line-height:30px;position:relative;"></span>')
...
//直播辅导介绍方法
function liveVideoPlay(videoUrl, flashplayerUrl) {
    var _html = '<script type="text/javascript">' + 'var swfVersionStr = "10.2.0";' + 'var xiSwfUrlStr = "playerProductInstall.swf";' + 'var flashvars = {};' + 'flashvars.url = "' + videoUrl + '";' + 'flashvars.autoPlay = "true";' + 'var params = {};' + 'params.quality = "high";' + 'params.bgcolor = "#666666";' + 'params.wmode = "transparent";' + 'params.allowscriptaccess = "sameDomain";' + 'params.allowfullscreen = "true";' + 'var attributes = {};' + 'attributes.id = "EncryptPlayerPop";' + 'attributes.name = "EncryptPlayer";' + 'attributes.align = "middle";' + 'swfobject.embedSWF("' + flashplayerUrl + '", "flashContent", "530px", "314px", swfVersionStr, xiSwfUrlStr, flashvars, params, attributes);' + 'swfobject.createCSS("#flashContent", "display:block;text-align:left;");' + '<\/script>';

    _html += '<div class="player_live">' + '<div id="VideoPlay" style="width:530px;height:314px">' + '<div class="lyrics-con-video" id="flashContent">' + '<script type="text/javascript">' + 'var pageHost = ((document.location.protocol == "https:") ? "https://" :   "http://"); ' + 'document.write("<a href=\'http://www.adobe.com/go/getflashplayer\'><img src=\'" + pageHost + "www.adobe.com/images/shared/download_buttons/get_flash_player.gif\' alt=\'Get Adobe Flash player\' /></a>" ); ' + '<\/script>' + '<\/div>' + '<\/div>' + '<\/div>';
    var pageVideo = $('#EncryptPlayer');
