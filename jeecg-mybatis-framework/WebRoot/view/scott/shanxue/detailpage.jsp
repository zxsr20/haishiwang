...
rel="stylesheet" type="text/css" href="<%=basePath%>/css/page_listen.css"><LINK 
rel="stylesheet" type="text/css" href="<%=basePath%>/css/page_home.css"><LINK 
...
<UL>
 <c:forEach items="${coursetype}" var="user"
							...
							</c:forEach></UL></DIV><!-- 内容 start-->
<DIV id="container"><LINK rel="stylesheet" type="text/css" href="" 
media="all">
<SCRIPT src=""></SCRIPT>
...
<H2>${course.sxCourse.name} <STRONG></STRONG></H2>
<DIV class="content_player">
<DIV class="player_list">
<DIV class="list_title"><SPAN style="padding-left: 20px;">课程内容</SPAN></DIV><!-- 课程目录列表-->
<DIV class="ui_scrolls scrolls_free">
<DIV style="display: block;" id="outline1" class="scrolls_item">
<DIV class="scrolls_con">
<UL class="select_list">
  <LI class="scrolls_title" title="${course.sxCourse.name}">
  										${course.sxCourse.name}									</LI>
  ...
  <c:forEach items="${course.sxPrograms}" var="user"
								...
								<LI  class="current">
  <P><EM class="icon icon_live_gary">节</EM><A title="影响学习效果的三大要素" href="newdetail.do?course_id=${user.course_id}&id=${user.id}"><c:out value="${fn:substring(user.name, 0, 10)}" /></A></P></LI>
  								</c:if>
...
class="document_download gray" disabled="">下载本课程讲义</A>
<SCRIPT>function ajaxCourseInformation(urlStr,urlKey){
					if(urlKey == '' || urlStr == ''){
						alert('参数不合法');
						return false;
					}
					...
					$.ajax({
						...
						type:'post',
						dataType:'html',
						data:{courseId:urlStr,urlKey:urlKey},
						success: function (data) {
//							alert(data);return false;
							var resData = xue.ajaxCheck.HTML(data);
							if(resData){
								xue.win({
...
<DL>
  ...
  <UL class="avatar_items">
    <LI class="ui_userinfo" data-params="userId=559" data-id="559">${course.sxCourse.teacher_name}</LI></UL></DIV></DIV></DD></DL>
<DIV class="listen_num"><STRONG>196889</STRONG><SPAN>播放</SPAN></DIV>
<DIV class="collect"><A id="collectId" onclick="collect(6836)" href="javascript:void(0);">收藏课程</A></DIV></DIV></DIV></DIV>
<SCRIPT>
		function collect(id){
			...
			$.ajax({
				url :'sx_collect_course.do',
					...
					success: function (msg) {//alert(data);return false;
						if (msg.success){
							$('#collectId').addClass('collect_current').attr('onclick','');
						}else{
							alert(msg.msg);
						}
					},
...
<UL id="course_tab" class="nav_links">
  <LI class="current" data-params="" data-url=""><A href="javascript:void(0);" 
  data-params="" data-url="">课程详情</A></LI>
  <LI class=" last" data-params="" data-url=""><A href="javascript:void(0);" 
  data-params="" data-url="">课程评价<STRONG>(2721)</STRONG></A></LI></UL></DIV>
<DIV id="course_info_box"><!--课程详情-->
<DIV style="display: block;" id="course_content" class="course_detail"><!--课程详情-->
<DL>
  <DT>课程更新时间</DT>
  <DD>				本课程更新于 <FONT color="red">${course.sxCourse.update_date}</FONT>。			</DD></DL>
<DL>
  <DL>
    <DT class="ui_infor_title_1">详情</DT>
    <DD class="agg"></DD>
    <DD class="agg">${course.sxCourse.description}</DD></DL></DL>
<!-- 课程详情 end--></DIV><!-- 课程详情 end--><!-- 课程评价 start-->
<DIV id="comment" class="course_detail" name="comment">
<DIV class="detail_login">														登录后才可以发表评价哦！<A href="http://login.xueersi.com/user/login/aHR0cDovL3d3dy54dWVlcnNpLmNvbS9vcGVuLzY4MzYvMS0xP2lzQ29tbWVudCNjb21tZW50">立即登录</A></DIV>
<P>&nbsp;</P>
<DIV id="comm_page_list" class="ui_course_consult"></DIV>
<DIV id="comm_pages" class="comm_pages" data-url="ajaxPagesUrl" 
data-pages="0"></DIV></DIV><!-- 课程评价 end --></DIV></DIV></DIV>
<DIV class="layout_side right w_240 mtop_20"><!-- 相关课程--><!-- 浏览过的记录-->
<DIV class="panel_listen">
...
<DIV class="ui_avatar">
<UL class="avatar_items">
  <LI class="ui_userinfo" data-params="userId=559" data-id="559">${course.sxCourse.teacher_name}</LI></UL></DIV>
<DIV class="cou_detail">
<DIV class="cou_text"><A href="javascript:void(0)" target="_blank">${course.sxCourse.name}</A></DIV>
<DIV class="cou_info"><SPAN><EM 
class="icon icon_gray_user"></EM>9721					</SPAN><SPAN><EM class="icon icon_thumbs_up"></EM>
						9					</SPAN></DIV></DIV></DIV>
<DIV class="listen_cou border_none">
<DIV class="ui_avatar">
<UL class="avatar_items">
  <LI class="ui_userinfo" data-params="userId=757" data-id="757"><A class="userpic" 
  href="javascript:void(0)" target="_blank"><IMG alt="" src=""></A></LI></UL></DIV>
<DIV class="cou_detail">
<DIV class="cou_text"><A href="javascript:void(0)" target="_blank">四升五年级“畅享语文”成长计划年卡（17-20级）</A></DIV>
<DIV class="cou_info"><SPAN><EM 
				...
				if(result.sign == 1){
					...
					xue.pages('comm_pages').config({
						...
						callback : function(current){
							xue.pages('comm_pages').go(current);
							ajaxCommPages(url, params, current);
							$('#comm_pages li').off('click');
						}
					});
				}else{
//					alert(result.sign);
	...
	function ajaxCommPages(url, course_id,page){
		$.ajax({
			url : url,
			type: 'POST',
			data : {courseId:course_id,curpage:page},
			dataType:'json',
			success: function(result){
				var box = $('#comm_page_list');
				if(result.sign == 1){
					...
					$.each(result.msg.data, function(k, v){
						tpl += '<div class="comment_detail_list">';
							...
							tpl += "<div class='user_text_box'>";
								tpl += '<p class="text"> '+ v[0]['content'] + '</p>';
								tpl += '<p class="time"> 发表于 '+ v[0]['create_time'] + '</p>';
								tpl += '<span class="corner_bg"></span>';
								if(v[0]['is_cream']==1){
									tpl += '<span class="added_icon"></span>';
								}
